...
{
	if ($title!=''&&$cat!=''){
		$ret = $xoopsDB -> query("SELECT a.lid, a.cid FROM "
                            . $xoopsDB->prefix("xtorrent_downloads")
                            . " a INNER JOIN "
                            . $xoopsDB->prefix("xtorrent_cat")
                            . " b ON a.cid = b.cid WHERE a.title LIKE '"
                            . $title
                            . "' AND b.title LIKE '"
                            . $cat
                            . "'");
		...
		$ret = $xoopsDB -> query("SELECT a.title, b.title as cat_title FROM "
                            . $xoopsDB->prefix("xtorrent_downloads")
                            . " a INNER JOIN "
                            . $xoopsDB->prefix("xtorrent_cat")
                            . " b ON a.cid = b.cid WHERE a.lid = '"
                            . $lid
                            . "'");
...
$sql    = "SELECT lid, cid, title, published FROM "
           . $xoopsDB->prefix('xtorrent_downloads')
           . " WHERE submitter = "
           . $down_arr['submitter']
 ...
 * User reviews
 */
$sql_review    = "SELECT * FROM " . $xoopsDB->prefix('xtorrent_reviews') . " WHERE lid = " . $down_arr['lid'] . " AND submit = 1";
...
if ($poll['torrent']+($xoopsModuleConfig['poll_torrent_time']*60)<time()&&$xoopsModuleConfig['poll_torrent']==1){
	$rt = poll_torrent($down['id']);
}

if ($poll['tracker']+($xoopsModuleConfig['poll_tracker_time']*60)<time()&&$xoopsModuleConfig['poll_tracker']==1){
