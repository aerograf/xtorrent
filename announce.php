...
if ($valid[0] == 0||empty($valid)) { err("Invalid passkey or secret! Re-download the .torrent from ".XOOPS_URL); }
...
if (!isset($self))
{
	//$rt=debugtxt("start self 2", $filename);
		...
		$rz = $xoopsDB->queryF("SELECT id, uploaded, downloaded FROM ".$xoopsDB->prefix('xtorrent_users')." WHERE passkey=".sqlesc($passkey)." AND enabled = 'yes' and secret = ".sqlesc(sha1(xtorrent_get_base_domain(gethostbyaddr($_SERVER['REMOTE_ADDR']))))." ORDER BY last_access DESC LIMIT 1") or err("Tracker error 2");
...
else
{
	...
	else
	{
		...
		if ($xoopsDB->getRowsNum($rt))
		{
			...
			$sql = "UPDATE ".$xoopsDB->prefix('xtorrent_peers')." SET connectable = '$connectable', torrent = '$torrentid', peer_id = " . sqlesc($peer_id) . ", ip = " . sqlesc($ip) . ", port = '$port', uploaded = '$uploaded', downloaded = '$downloaded', to_go = '$left', started = NOW(), last_action = NOW(), seeder = '$seeder', userid = '$userid', agent = " . sqlesc(addslashes($agent)) . ", uploadoffset = '$uploaded', downloadoffset = '$downloaded', passkey = " . sqlesc($passkey) . ") WHERE id = $pid";
		} else {
			$sql = "INSERT INTO ".$xoopsDB->prefix('xtorrent_peers'). " (connectable, torrent, peer_id, ip, port, uploaded, downloaded, to_go, started, last_action, seeder, userid, agent, uploadoffset, downloadoffset, passkey) VALUES ('$connectable', '$torrentid', " . sqlesc(addslashes($peer_id)) . ", " . sqlesc($ip) . ", '$port', '$uploaded', '$downloaded', '$left', NOW(), NOW(), '$seeder', '$userid', " . sqlesc(addslashes($agent)) . ", '$uploaded', '$downloaded', " . sqlesc($passkey) . ")";			
		}
