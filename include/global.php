...
function get_row_count($table, $suffix = '')
{
  ...
  //($r = mysqli_query("SELECT COUNT(*) FROM $table$suffix")) or die(mysqli_error());
...
function stdmsg($heading, $text)
{
  ...
  if ($heading)
  print('<h2>' . $heading . '</h2>');
  print("<table style='width:100%;border:1;padding:10;'><tr><td class='text'>");
...
{
  print("<table style='background:blue;text-align:left;padding:10;border:0;'>"
...
function format_quotes($s)
{
  while ($old_s != $s)
  {
	  ...
	  if (false === $open)
    {
	  ...
	  //[quote]Text[/quote]
	  $quote = preg_replace("/\[quote\]\s*((\s|.)+?)\s*\[\/quote\]\s*/i", "<p class=sub><b>Quote:</b></p><table class=main style='border:1px;padding:10;'><tr><td style='border:1px black dotted'>\\1</td></tr></table><br>", $quote);

	  //[quote=Author]Text[/quote]
	  $quote = preg_replace("/\[quote=(.+?)\]\s*((\s|.)+?)\s*\[\/quote\]\s*/i", "<p class=sub><b>\\1 wrote:</b></p><table class=main style='border:1px;padding:10;'><tr><td style='border:1px black dotted'>\\2</td></tr></table><br>", $quote);
...
function format_comment($text, $strip_html = true)
{
  ...
  // (What took us so long? :blush:)- wyz
	...
	$s = preg_replace("/\[img\]([^\s'\"<>]+?)\[\/img\]/i", "<img style='border:0;' src=\"\\1\">", $s);

	// [img=http://www/image.gif]
	$s = preg_replace("/\[img=([^\s'\"<>]+?)\]/i", "<img style='border:0;' src=\"\\1\">", $s);
...
//	$s = format_local_urls($s);
  ...
  {
		$s = str_replace($code, "<img style='border:0;' src=\"/pic/smilies/" . $url . " alt=\"" . htmlspecialchars($code) . '">', $s);
  }

	reset($privatesmilies);
	while (list($code, $url) = each($privatesmilies))
  {
		$s = str_replace($code, "<img style='border:0;' src=\"/pic/smilies/" . $url . '>', $s);
  }
...
function is_valid_id($id)
{
  ...
  {
    print("<table class='main' style='width:750px;border:0;'>" . "<tr><td class='embedded'>");
  }
  ...
  function begin_frame($caption = '', $center = false, $padding = 10)
  {
    ...
    print("<table style='width:100%;border:1px;padding:" . $padding . ";'><tr><td style='text-align:" . $tdextra . ';>');
