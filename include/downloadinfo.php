...
if (isset($down_arr['screenshot']))
{
    ...
    if (!empty($down_arr['screenshot']) && file_exists(XOOPS_ROOT_PATH . '/' . $xoopsModuleConfig['screenshots'] . '/' . xoops_trim($down_arr['screenshot'])))
    {
        ...
        {
          $down['screenshot_thumb'] = down_createthumb($down['screenshot_full'], $xoopsModuleConfig['screenshots'], 'thumbs', $xoopsModuleConfig['shotwidth'], $xoopsModuleConfig['shotheight'], $xoopsModuleConfig['imagequality'], $xoopsModuleConfig['updatethumbs'], $xoopsModuleConfig['keepaspect']);
        }
        else
        {
...
$down['homepage'] = (!$down_arr['homepage'] || 'https://' == $down_arr['homepage']) ? '' : $myts->htmlSpecialChars(trim($down_arr['homepage']));
...
$down['mirror']   = $down['mirror'] ? "<a href='" . $down['mirror'] . "' target='_blank'>" . _MD_XTORRENT_MIRRORSITE . '</a>' : _MD_XTORRENT_NOTSPECIFIED;
...
$down['price']       = ($down_arr['price'] != 0) ? (int)$down_arr['price'] : _MD_XTORRENT_PRICEFREE;
