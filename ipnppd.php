...
while (!$dbg && !$ERR && !feof($fp)) 
{
        ...
        if (strcmp ($res, "VERIFIED") == 0)
        {
                ...
                if( strcmp($payment_status, "Refunded") == 0)
                {
                        ...
                        $insertSQL = "INSERT INTO ".$xoopsDB->prefix("xtorrent_payments")." (`txn_id`,`business`,`item_name`, `item_number`, `quantity`, `invoice`, `custom`, `memo`, `tax`, `option_name1`, `option_selection1`, `option_name2`, `option_selection2`, `payment_status`, `payment_date`, `txn_type`, `mc_gross`, `mc_fee`, `mc_currency`, `settle_amount`, `exchange_rate`, `first_name`, `last_name`, `address_street`, `address_city`, `address_state`, `address_zip`, `address_country`, `address_status`, `payer_email`, `payer_status`, `torrent`, `passkey`, `userid`) VALUES ('".$_REQUEST['txn_id']."','".$_REQUEST['business']."','".$_REQUEST['item_name']."','".$_REQUEST['item_number']."','".$_REQUEST['quantity']."','".$_REQUEST['invoice']."','".$_REQUEST['custom']."','".$_REQUEST['memo']."','".$_REQUEST['tax']."','".$_REQUEST['option_name1']."','".$_REQUEST['option_selection1']."','".$_REQUEST['option_name2']."','".$_REQUEST['option_selection2']."','".$_REQUEST['payment_status']."','".strftime('%Y-%m-%d %H:%M:%S',strtotime($_REQUEST['payment_date']))."','".$_REQUEST['txn_type']."','$mc_gross','$mc_fee','".$_REQUEST['mc_currency']."','".$_REQUEST['settle_amount']."','".$_REQUEST['exchange_rate']."','".$_REQUEST['first_name']."','".$_REQUEST['last_name']."','".$_REQUEST['address_street']."','".$_REQUEST['address_city']."','".$_REQUEST['address_state']."','".$_REQUEST['address_zip']."','".$_REQUEST['address_country']."','".$_REQUEST['address_status']."','".$_REQUEST['payer_email']."','".$_REQUEST['payer_status']."','".$_REQUEST['item_number']."','".$_REQUEST['custom']."','".$iscrape[0]."')";
                ...
                if( (strcmp($payment_status, "Completed") == 0) && ((strcmp($txn_type, "web_accept")== 0) || (strcmp($txn_type, "send_money")== 0)) )
                {
                        ...
                        if( $lp ) fputs($lp, $payer_email . " " . $payment_status . " " . $_REQUEST['payment_date'] . "\n");
                        ...
                        $insertSQL = "INSERT INTO ".$xoopsDB->prefix("xtorrent_payments")." (`txn_id`,`business`,`item_name`, `item_number`, `quantity`, `invoice`, `custom`, `memo`, `tax`, `option_name1`, `option_selection1`, `option_name2`, `option_selection2`, `payment_status`, `payment_date`, `txn_type`, `mc_gross`, `mc_fee`, `mc_currency`, `settle_amount`, `exchange_rate`, `first_name`, `last_name`, `address_street`, `address_city`, `address_state`, `address_zip`, `address_country`, `address_status`, `payer_email`, `payer_status`, `torrent`, `passkey`, `userid`) VALUES ('".$_REQUEST['txn_id']."', '".$_REQUEST['business']."', '".$_REQUEST['item_name']."', '".$_REQUEST['item_number']."', '".$_REQUEST['quantity']."', '".$_REQUEST['invoice']."', '".$_REQUEST['custom']."', '".$_REQUEST['memo']."', '".$_REQUEST['tax']."', '".$_REQUEST['option_name1']."', '".$_REQUEST['option_selection1']."', '".$_REQUEST['option_name2']."', '".$_REQUEST['option_selection2']."', '".$_REQUEST['payment_status']."', '".strftime('%Y-%m-%d %H:%M:%S', strtotime($_REQUEST['payment_date']))."', '".$_REQUEST['txn_type']."', '".$_REQUEST['mc_gross']."', '".$_REQUEST['mc_fee']."', '".$_REQUEST['mc_currency']."', '".$_REQUEST['settle_amount']."', '".$_REQUEST['exchange_rate']."', '".$_REQUEST['first_name']."', '".$_REQUEST['last_name']."', '".$_REQUEST['address_street']."', '".$_REQUEST['address_city']."', '".$_REQUEST['address_state']."', '".$_REQUEST['address_zip']."', '".$_REQUEST['address_country']."', '".$_REQUEST['address_status']."', '".$_REQUEST['payer_email']."', '".$_REQUEST['payer_status']."','".$_REQUEST['item_number']."','".$_REQUEST['custom']."','".$iscrape[0]."')";
...
if( $log )
{
        ...
        $sql     = "INSERT INTO " . $xoopsDB->prefix("xtorrent_translog")." VALUES (NULL,'" . strftime('%Y-%m-%d %H:%M:%S',mktime()) . "', '"
                                  . strftime('%Y-%m-%d %H:%M:%S', strtotime($_REQUEST['payment_date'])) . "','" . addslashes($log) . "')";
