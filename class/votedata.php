...
class XtorrentVotedataHandler extends XoopsObjectHandler {
	var $db;
	var $db_table;
	var $perm_name = 'xtorrent_votedata_';
	var $obj_class = 'votedataResource';
	...
	function insert($votedata, $force = false){
	...
	function &getObjects($criteria = null, $fields='*', $ratingid_as_key = false){
		$ret   = [];
		$limit = $start = 0;
	...
	function deleteTorrentPermissions($ratingid, $mode = "view"){
		global $xoopsModule;
	...
	function insertTorrentPermissions($ratingid, $group_ids, $mode = "view"){
		global $xoopsModule;
	...
	function &getPermittedTorrents($votedata, $mode = "view"){
	...
	function getSingleTorrentPermission($ratingid, $mode = "view"){
		global $xoopsUser, $xoopsModule;
		$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : 3;
