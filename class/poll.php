    ...
    public function insert($poll, $force = false)
    {
        ...
        if ($poll->isNew() || empty($id)) {
            ...
            $sql = sprintf('INSERT INTO %s (
				...
				%u, %s, %s
				)', $this->db_table, $this->db->quoteString($lid), $this->db->quoteString($torrent), $this->db->quoteString($tracker));
        } else {
            $sql = sprintf('UPDATE %s SET
				`torrent` = %s,
				`tracker` = %s,
				 WHERE lid = %s', $this->db_table, $this->db->quoteString($torrent), $this->db->quoteString($tracker), $this->db->quoteString($lid));
        }
