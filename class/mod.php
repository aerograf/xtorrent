...
class XtorrentModHandler extends XoopsObjectHandler {
	var $db;
	var $db_table;
	var $perm_name = 'xtorrent_mod_';
	var $obj_class = 'modResource';
	...
	function insert($mod, $force = false){
	...
	function getObjects($criteria = null, $fields='*', $requestid_as_key = false){
		$ret   = [];
		$limit = $start = 0;
	...
	function deleteTorrentPermissions($requestid, $mode = "view"){
		global $xoopsModule;
	...
	function insertTorrentPermissions($requestid, $group_ids, $mode = "view"){
		global $xoopsModule;
	...
	function getPermittedTorrents($mod, $mode = "view"){
	...
	function getSingleTorrentPermission($requestid, $mode = "view"){
		global $xoopsUser, $xoopsModule;
		$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : 3;
