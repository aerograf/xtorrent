...
class XtorrentCatHandler extends XoopsObjectHandler {
	var $db;
	var $db_table;
	var $perm_name = 'xtorrent_cat_';
	var $obj_class = 'catResource';
	...
	function insert(&$cat, $force = false){
	...
	function getObjects($criteria = null, $fields='*', $cid_as_key = false){
		$ret   = [];
		$limit = $start = 0;
	...
	function deleteTorrentPermissions($cid, $mode = "view"){
		global $xoopsModule;
	...
	function insertTorrentPermissions($cid, $group_ids, $mode = "view"){
		global $xoopsModule;
	...
	function getPermittedTorrents($cat, $mode = "view"){
	...
	function getSingleTorrentPermission($cid, $mode = "view"){
		global $xoopsUser, $xoopsModule;
		$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : 3;
