<?php
// $Id: pcm3108.enumerator.php 1.1.0 - pcm3108 2009-08-15 9:22:20 wishcraft $
//  ------------------------------------------------------------------------ //
...
if (!class_exists('pcm3108_enumerator')) {
    error_reporting(0);
    
    class pcm3108_enumerator extends pcm3108
    {
        public $elekey;
        public $base;
        public $len;
            ...
            foreach ($base->base as $key => $data) {
                if (strlen((string)$data)==1) {
                    if (strlen(bindec(ord($data)))==5) {
                        $offset = ["ice" => (int)substr(decbin(ord($data)), 4, 1),
                        ...
                        $offset = ["icf" => (int)substr(decbin(ord($data)), 3, 1),
                                        "ice" => (int)substr(decbin(ord($data)), 2, 1),
                                        "icd" => (int)substr(decbin(ord($data)), 5, 1),
                        ...
                        $offset = ["ica" => (int)substr(decbin(ord($data)), 5, 1),
                                        "icb" => (int)substr(decbin(ord($data)), 6, 1),
                                        "icc" => (int)substr(decbin(ord($data)), 1, 1),
                                        "icd" => (int)substr(decbin(ord($data)), 2, 1),
                                        "ice" => (int)substr(decbin(ord($data)), 3, 1),
                                        "icf" => (int)substr(decbin(ord($data)), 5, 1)];
                    }
                } else {
                    $offset = ["ica" => (int)substr(decbin(ord(substr($key, strlen($key)-1, 1))), 1, 1),
                                    "icb" => (int)substr(decbin(ord(substr($key, strlen($key)-1, 1))), 6, 1),
                                    "icc" => (int)substr(decbin(ord(substr($key, strlen($key)-1, 1))), 3, 1),
                                    "icd" => (int)substr(decbin(ord(substr($key, strlen($key)-1, 1))), 4, 1),
                                    "ice" => (int)substr(decbin(ord(substr($key, strlen($key)-1, 1))), 6, 1),
