...
class XtorrentTransactionsHandler extends XoopsObjectHandler
{
        ...
        if (isset($transactions)) {
            ...
            if ($transactions = $this->getObjects($criteria, 'home_list')) {
                ...
                foreach ($transactions as $f) {
                    if (false !== $this->permHandler->checkRight($this->perm_name . $mode, $f->getVar('xtid'), $groups, $xoopsModule->getVar('mid'))) {
                        $ret[] = $f;
                        unset($f);
                    }
                }
    ...
    public function getSingleTorrentPermission($xtid, $mode = 'view')
    {
        global $xoopsUser, $xoopsModule;
        $groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : 3;
        if (false !== $this->permHandler->checkRight($this->perm_name . $mode, $xtid, $groups, $xoopsModule->getVar('mid'))) {
            return true;
        }
        return false;
    }
