...
class XtorrentBrokenHandler extends XoopsObjectHandler {
	var $db;
	var $db_table;
	var $perm_name = 'xtorrent_broken_';
	var $obj_class = 'brokenResource';
	...
	function insert(&$broken, $force = false){
	...
	function getObjects($criteria = null, $fields='*', $reportid_as_key = false){
		$ret   = [];
		$limit = $start = 0;
	...
	function deleteTorrentPermissions($reportid, $mode = "view"){
		global $xoopsModule;
	...
	function insertTorrentPermissions($reportid, $group_ids, $mode = "view"){
		global $xoopsModule;
	...
	function getPermittedTorrents($broken, $mode = "view"){
	...
	function getSingleTorrentPermission($reportid, $mode = "view"){
		global $xoopsUser, $xoopsModule;
		$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : 3;
